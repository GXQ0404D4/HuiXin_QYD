...
import com.ktcn.dao.providerSQL.reportProviderSQL.weekReport.AirStatementsSQL;
...
@Mapper
@Repository
public interface AirStatementsDao {
	// 获取数据总条数
	...
	// 时间区间查询总条数
	@SelectProvider(method = "findCountByTime", type = AirStatementsSQL.class)
	int findCountByTime(String current_timeA, String current_timeB);
	// 根据时间区间查询离心机日报表信息
	@SelectProvider(method = "findByTime", type = AirStatementsSQL.class)
