		...
		String sql = new SQL() {
			// 编写拼接SQL代码块
			{
				...
				if (timeA!=null && !"".equals(timeA)) {
					...
					// 任务名称
					String s = "`mt_time` BETWEEN '"+ timeA +"' AND '"+ nowDate +"'";
					WHERE(s);
				}
				if (timeB!=null && !"".equals(timeB)) {
					// 任务名称
					String s = "`mt_time` BETWEEN '1970-01-01 00:00:00' AND '"+ timeB +"'";
					WHERE(s);
				}
				if (timeA==null && "".equals(timeA) && timeB==null && "".equals(timeB)) {
					Date date = new Date();
					...
					String nowDate = format.format(date);
					// 任务名称
					String s = "`mt_time` BETWEEN '1970-01-01 00:00:00' AND '"+ nowDate +"'";
					WHERE(s);
				}
			}
		}.toString();
