 ...
 */
@RestController
public class AirStatementsController {
	
	@Resource
	private AirStatementsService airStatementsService;
	
	// 查看全部
	@RequestMapping("findAllas")
	@SysLog(logModule = "离心机报表", logName = "查看全部")
	public Map<String, Object> findAll(HttpServletRequest request) {
		try {
			HttpSession session = request.getSession();
			session.removeAttribute("TimeAas");
			session.removeAttribute("TimeBas");
		} catch (Exception e) {
	...
	// 查看离心机日报表数据
	@RequestMapping("findas")
	@SysLog(logModule = "离心机报表", logName = "条件查询")
	public Map<String, Object> findEM(String pageSize, String current_timeA, String current_timeB,
		...
		if ((current_timeA == null && current_timeB == null) || (current_timeA.equals("") && current_timeB.equals(""))) {
			if (session.getAttribute("TimeAas") == null && session.getAttribute("TimeBas") == null) {
				// 查询数据总条数
				count = airStatementsService.findTotal();
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = airStatementsService.findAllByIndex(x);
			} else {
				// 获取日期查询信息
				String a1 = (String) session.getAttribute("TimeAas");
				String a2 = (String) session.getAttribute("TimeBas");
				// 查询数据总条数
				count = airStatementsService.findCountByTime(a1, a2);
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = airStatementsService.findByTime(a1, a2, x);
			}
		} else {
			// 写入查询值
			session.setAttribute("TimeAas", current_timeA);
			session.setAttribute("TimeBas", current_timeB);
			// 查询数据总条数
			count = airStatementsService.findCountByTime(current_timeA, current_timeB);
			// 获取总页数
			countPage = PageReade.getCountPage(count, 10);
			// 获取查询起始条数
			x = PageReade.getPageSize(i, count, 10);
			// 获取查询数据
			list = airStatementsService.findByTime(current_timeA, current_timeB, x);
		}
	...
	// Excel导出_离心机日报表数据
	@RequestMapping(value = "export_as")
