 ...
 */
@RestController
public class ScrewyController {
	
	@Resource
	private ScrewyService screwyService;
	
	// 查看全部
	@RequestMapping("yfindAllsc")
	@SysLog(logModule = "螺杆机年报表", logName = "查看全部")
	public Map<String, Object> findAll(HttpServletRequest request) {
		try {
			HttpSession session = request.getSession();
			session.removeAttribute("TimeAscy");
			session.removeAttribute("TimeBscy");
		} catch (Exception e) {
	...
	// 查看螺杆机日报表数据
	@RequestMapping("yfindsc")
	@SysLog(logModule = "螺杆机年报表", logName = "条件查询")
	public Map<String, Object> findEM(String pageSize, String current_timeA, String current_timeB,
		...
		if ((current_timeA == null && current_timeB == null) || (current_timeA.equals("") && current_timeB.equals(""))) {
			if (session.getAttribute("TimeAscy") == null && session.getAttribute("TimeBscy") == null) {
				// 查询数据总条数
				count = screwyService.findTotal();
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = screwyService.findAllByIndex(x);
			} else {
				// 获取日期查询信息
				String a1 = (String) session.getAttribute("TimeAscy");
				String a2 = (String) session.getAttribute("TimeBscy");
				// 查询数据总条数
				count = screwyService.findCountByTime(a1, a2);
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = screwyService.findByTime(a1, a2, x);
			}
		} else {
			// 写入查询值
			session.setAttribute("TimeAscy", current_timeA);
			session.setAttribute("TimeBscy", current_timeB);
			// 查询数据总条数
			count = screwyService.findCountByTime(current_timeA, current_timeB);
			// 获取总页数
			countPage = PageReade.getCountPage(count, 10);
			// 获取查询起始条数
			x = PageReade.getPageSize(i, count, 10);
			// 获取查询数据
			list = screwyService.findByTime(current_timeA, current_timeB, x);
		}
	...
	// Excel导出_螺杆机日报表数据
	@RequestMapping(value = "yexport_sc")
	public void ElectricityReport_export(HttpServletRequest request, HttpServletResponse response,
		...
		HSSFWorkbook wb = screwyService.downloadExcel(isEntity);
