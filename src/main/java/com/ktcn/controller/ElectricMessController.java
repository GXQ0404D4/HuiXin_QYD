...
public class ElectricMessController {
	...
	public Map<String, Object> findAll(HttpServletRequest request) {
		try {
			...
			session.removeAttribute("TimeAem");
			session.removeAttribute("TimeBem");
		} catch (Exception e) {
	...
	public Map<String, Object> findEM(String pageSize, String current_timeA, String current_timeB,
		...
		if ((current_timeA == null && current_timeB == null) || (current_timeA.equals("") && current_timeB.equals(""))) {
			if (session.getAttribute("TimeAem") == null && session.getAttribute("TimeBem") == null) {
				...
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = electricMessService.findByTime(a1, a2, x);
			}
		} else {
			// 写入查询值
			session.setAttribute("TimeAem", current_timeA);
			session.setAttribute("TimeBem", current_timeB);
			// 查询数据总条数
			count = electricMessService.findCountByTime(current_timeA, current_timeB);
			// 获取总页数
			countPage = PageReade.getCountPage(count, 10);
			// 获取查询起始条数
			x = PageReade.getPageSize(i, count, 10);
			// 获取查询数据
			list = electricMessService.findByTime(current_timeA, current_timeB, x);
		}
