...
public class ElectricReportController {
	...
	// 查看全部
	@RequestMapping("findAller")
	@SysLog(logModule = "电量报表", logName = "查看全部")
	public Map<String, Object> findAll(HttpServletRequest request) {
		try {
			HttpSession session = request.getSession();
			session.removeAttribute("TimeAer");
			session.removeAttribute("TimeBer");
		} catch (Exception e) {
	...
	@SysLog(logModule = "电量报表", logName = "条件查询")
	public Map<String, Object> findEM(String pageSize, String current_timeA, String current_timeB,
		...
		if ((current_timeA == null && current_timeB == null) || (current_timeA.equals("") && current_timeB.equals(""))) {
			if (session.getAttribute("TimeAer") == null && session.getAttribute("TimeBer") == null) {
				// 查询数据总条数
				count = electricReportService.findTotal();
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = electricReportService.findAllByIndex(x);
			} else {
				// 获取日期查询信息
				String a1 = (String) session.getAttribute("TimeAer");
				String a2 = (String) session.getAttribute("TimeBer");
				// 查询数据总条数
				count = electricReportService.findCountByTime(a1, a2);
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = electricReportService.findByTime(a1, a2, x);
			}
		} else {
			// 写入查询值
			session.setAttribute("TimeAer", current_timeA);
			session.setAttribute("TimeBer", current_timeB);
			// 查询数据总条数
			count = electricReportService.findCountByTime(current_timeA, current_timeB);
			// 获取总页数
			countPage = PageReade.getCountPage(count, 10);
			// 获取查询起始条数
			x = PageReade.getPageSize(i, count, 10);
			// 获取查询数据
			list = electricReportService.findByTime(current_timeA, current_timeB, x);
		}
	...
	// Excel导出_电量报表数据
	@RequestMapping(value = "export_er")
