...
public class OPCAddressInsert {
	...
	public void GetOPCInsert(Map<String, Float> map1, Map<String, Boolean> map2, Map<String, Long> map3,
		...
		for (String key : map1.keySet()) {
			...
			System.out.println((a++) + "---" + "key:" + key + " vlaue:" + value);
		}
		for (String key : map3.keySet()) {
			String value = map3.get(key).toString();//

			System.out.println((i++) + "---" + "key:" + key + " vlaue:" + value);
		}
		for (String key : map2.keySet()) {
			String value = map2.get(key).toString();//

			System.out.println((d++) + "---" + "key:" + key + " vlaue:" + value);
		}
		for (String string : allAddress) {
			System.out.println("地址值" + (v++) + "---" + string);
		}
		...
		switch_output_value.setS15(map2.get("DO-3#KYJZD/SD")); // Switch_output 数据
		System.out.println(switch_output_value);
		DataMasterTable.getswitchOutput(switch_output_value);

		// update zidu //switch_input 数据
		...
		switch_input_value.setI32(map2.get("DI-3#KYJJZ/XZ")); // switch_input 数据
