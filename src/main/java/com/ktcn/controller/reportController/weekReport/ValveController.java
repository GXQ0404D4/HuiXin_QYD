...
import com.ktcn.service.reportService.weekReport.ValveService;
...
public class ValveController {
	...
	// 查看全部
	@RequestMapping("findAllva")
	@SysLog(logModule = "调节阀日报表", logName = "查看全部")
	public Map<String, Object> findAll(HttpServletRequest request) {
		try {
			HttpSession session = request.getSession();
			session.removeAttribute("TimeAva");
			session.removeAttribute("TimeBva");
		} catch (Exception e) {
	...
	// 查看螺杆机日报表数据
	@RequestMapping("findva")
	@SysLog(logModule = "调节阀日报表", logName = "条件查询")
	public Map<String, Object> findEM(String pageSize, String current_timeA, String current_timeB,
		...
		if ((current_timeA == null && current_timeB == null) || (current_timeA.equals("") && current_timeB.equals(""))) {
			if (session.getAttribute("TimeAva") == null && session.getAttribute("TimeBva") == null) {
				// 查询数据总条数
				count = valveService.findTotal();
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = valveService.findAllByIndex(x);
			} else {
				// 获取日期查询信息
				String a1 = (String) session.getAttribute("TimeAva");
				String a2 = (String) session.getAttribute("TimeBva");
				// 查询数据总条数
				count = valveService.findCountByTime(a1, a2);
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = valveService.findByTime(a1, a2, x);
			}
		} else {
			// 写入查询值
			session.setAttribute("TimeAva", current_timeA);
			session.setAttribute("TimeBva", current_timeB);
			// 查询数据总条数
			count = valveService.findCountByTime(current_timeA, current_timeB);
			// 获取总页数
			countPage = PageReade.getCountPage(count, 10);
			// 获取查询起始条数
			x = PageReade.getPageSize(i, count, 10);
			// 获取查询数据
			list = valveService.findByTime(current_timeA, current_timeB, x);
		}
	...
	// Excel导出_调节阀日报表数据
	@RequestMapping(value = "export_va")
	public void ElectricityReport_export(HttpServletRequest request, HttpServletResponse response,
		...
		HSSFWorkbook wb = valveService.downloadExcel(isEntity);
