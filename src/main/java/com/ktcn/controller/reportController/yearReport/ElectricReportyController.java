 ...
 */
@RestController
public class ElectricReportyController {
	
	@Resource
	private ElectricReportyService electricReportyService;
	
	// 查看全部
	@RequestMapping("yfindAller")
	@SysLog(logModule = "电量年报表", logName = "查看全部")
	public Map<String, Object> findAll(HttpServletRequest request) {
		try {
			HttpSession session = request.getSession();
			session.removeAttribute("TimeAery");
			session.removeAttribute("TimeBery");
		} catch (Exception e) {
	...
	// 查看电量报表数据
	@RequestMapping("yfinder")
	@SysLog(logModule = "电量年报表", logName = "条件查询")
	public Map<String, Object> findEM(String pageSize, String current_timeA, String current_timeB,
		...
		if ((current_timeA == null && current_timeB == null) || (current_timeA.equals("") && current_timeB.equals(""))) {
			if (session.getAttribute("TimeAery") == null && session.getAttribute("TimeBery") == null) {
				// 查询数据总条数
				count = electricReportyService.findTotal();
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = electricReportyService.findAllByIndex(x);
			} else {
				// 获取日期查询信息
				String a1 = (String) session.getAttribute("TimeAery");
				String a2 = (String) session.getAttribute("TimeBery");
				// 查询数据总条数
				count = electricReportyService.findCountByTime(a1, a2);
				// 获取总页数
				countPage = PageReade.getCountPage(count, 10);
				// 获取查询起始条数
				x = PageReade.getPageSize(i, count, 10);
				// 获取查询数据
				list = electricReportyService.findByTime(a1, a2, x);
			}
		} else {
			// 写入查询值
			session.setAttribute("TimeAery", current_timeA);
			session.setAttribute("TimeBery", current_timeB);
			// 查询数据总条数
			count = electricReportyService.findCountByTime(current_timeA, current_timeB);
			// 获取总页数
			countPage = PageReade.getCountPage(count, 10);
			// 获取查询起始条数
			x = PageReade.getPageSize(i, count, 10);
			// 获取查询数据
			list = electricReportyService.findByTime(current_timeA, current_timeB, x);
		}
	...
	// Excel导出_电量报表数据
	@RequestMapping(value = "yexport_er")
