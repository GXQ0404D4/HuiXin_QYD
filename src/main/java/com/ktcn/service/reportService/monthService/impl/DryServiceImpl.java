package com.ktcn.service.reportService.monthService.impl;
...
import com.ktcn.dao.report.monthDao.DryDao;
...
@Service
@Transactional
public class DryServiceImpl implements DryService {
	
	@Resource
	private DryDao dryDao;
	...
	public Map<String, Object> find(String pageSize, int sizeNum, String time1, String time2) {
		...
		// 查询数据总条数
		count = dryDao.findTotal(time1,time2);
		// 获取总页数
		countPage = PageReade.getCountPage(count, sizeNum);
		// 获取查询起始条数
		x = PageReade.getPageSize(i, count, sizeNum);
		// 获取数据
		List<DryingMachine> list = dryDao.find(time1,time2,x,sizeNum);
